...
<body class="h-full text-slate-200 overflow-hidden flex flex-col md:flex-row font-sans">
    ...
    <script>
        ...
        function renderTree() {
            ...
            // If searching, we might want to show a flat list or filtered tree?
            // Requirement says "déplie automatiquement l'arbre". So we keep tree structure.

            // However, we only render nodes that match OR have matching children.
            // Recursive render function

            function renderNode(account, level = 0) {
                const children = getChildren(account.id);
                const hasChildren = children.length > 0;
                const isExpanded = state.expanded.has(account.id) || state.searchQuery.length > 0; // Keep expanded if search active? Or rely on set.
                // Using the set for search expansion logic is cleaner.

                // Filter visibility based on search
                // Node is visible if:
                // 1. It matches query
                // 2. Any descendant matches query (so we can reach the match)
                // 3. Query is empty
                ...
                if (hasChildren && state.expanded.has(account.id)) {
                    ...
                    // childrenContainer.className = 'border-l border-slate-700 ml-4';
        ...
        function renderDetail() {
            ...
            account.example.rows.forEach(row => {
                ...
                tr.innerHTML = `
                    <td class="p-3 border-b border-slate-600 font-mono text-slate-300">${row.account}</td>
        ...
        mobileMenuBtn.addEventListener('click', () => {
             // Just focus search or something?
             // Sidebar is actually always visible on mobile BEHIND the main content if main content slides.
             // But layout says sidebar takes full screen on mobile.

             // CSS: Sidebar is absolute on mobile.
