    ...
    <main id="main-content" class="w-full md:w-2/3 lg:w-3/4 bg-slate-800 h-full overflow-y-auto relative transform transition-transform duration-300 translate-x-full md:translate-x-0 absolute md:relative inset-0 z-20">
        ...
        <div id="detail-view" class="hidden min-h-full">
            ...
            <div class="p-8 max-w-4xl mx-auto -mt-8 relative z-10 space-y-8">
                ...
                <!-- Example Container -->
                <div id="examples-container" class="space-y-8">
                    <!-- Example cards injected via JS -->
                </div>
    ...
    <script>
        ...
        function toggleNode(id) {
            if (state.expanded.has(id)) {
                ...
                // Accordion Logic: Close siblings before opening this one
                const account = getAccount(id);
                if (account) {
                    const siblings = getChildren(account.parent || null); // Get siblings (including self)
                    siblings.forEach(sibling => {
                         if (sibling.id !== id && state.expanded.has(sibling.id)) {
                             state.expanded.delete(sibling.id);
                         }
                    });
                }
        ...
        function selectAccount(id) {
            ...
            // Refined Accordion Logic on Jump:
            // When jumping to a new account, we want to clean up the tree.
            // But we must be careful not to collapse the path we just opened.
            // A simple strategy: Iterate all expanded nodes, if a node is NOT in the new path,
            // check if it is a sibling of something in the path. If so, collapse it.
            // Or simpler: Just leave it, the user might want context.
            // The prompt asked "replier l'ancien" (collapse the old one).
            // Let's implement a stricter cleanup: Close everything NOT in the path?
            // That might be too aggressive (closing unrelated root nodes).
            // Let's try: Close expanded nodes that are NOT in the path AND are siblings of nodes in the path.

            // Actually, simply invoking toggleNode logic downwards is hard.
            // Let's iterate all expanded IDs.
            const newExpanded = new Set();
            // We want to keep:
            // 1. The path to the selected node.
            // 2. Maybe the root nodes?

            // Let's stick to the path logic.
            // If we want "Accordion", we basically want only one branch open at each level.
            // So for every level of the new path, we ensure only that node is open.

            // Reset expanded to JUST the path + roots (optional)
            // But we need to know which siblings to close.

            // Better approach:
            // 1. Identify the path [Root, Child1, Child2...]
            // 2. For each node in the path:
            //    - Expand it.
            //    - Collapse all its siblings.

            let pathNode = account;
            while(pathNode) {
                state.expanded.add(pathNode.id);
                // Collapse siblings
                const parentId = pathNode.parent;
                const siblings = getChildren(parentId);
                siblings.forEach(sib => {
                    if (sib.id !== pathNode.id) {
                        state.expanded.delete(sib.id);
                    }
                });
                pathNode = getAccount(parentId);
            }
        ...
        function renderDetail() {
            ...
            // --- Examples Rendering ---
            examplesContainer.innerHTML = ''; // Clear previous

            const examples = account.examples || [];

            if (examples.length === 0) {
                 examplesContainer.innerHTML = '<p class="text-slate-500 italic">Aucun exemple spécifique.</p>';
            }

            examples.forEach((ex, index) => {
                const card = document.createElement('div');
                card.className = "bg-slate-700/50 backdrop-blur-sm border border-slate-600 rounded-xl p-6 shadow-xl";
                ...
                const titleHtml = `
                    <h3 class="text-lg font-semibold text-slate-200 mb-4 flex items-center">
                        ...
                        Exemple ${index + 1} : <span class="ml-2 text-green-300 italic">${ex.title}</span>
                    </h3>
                `;

                const tableContainer = document.createElement('div');
                tableContainer.className = "overflow-x-auto rounded-lg border border-slate-600";
                ...
                table.innerHTML = `
                    ...
                    <tbody class="bg-slate-700/30 divide-y divide-slate-600">
                    </tbody>
                `;
