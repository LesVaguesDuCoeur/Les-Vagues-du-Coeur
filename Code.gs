...
function apiGetConversations(token, email) {
  ...
  user.activeChats.forEach(chat => {
      ...
      if (typeof chat === 'string') {
          ...
          } catch(e) { changed = true; } // Removed if file missing
      } else {
          // Optimized Object
          // FIX: Check if file actually exists to avoid ghost chats
          let exists = true;
          try {
...
function apiExpireChat(token, email, chatId) {
    ...
    try {
        ...
        // Check permissions: Admin, Creator, Subscriber OR Participant?
        // User said: "celui qui a le crayont admin ou abonnement"
        const allowed = user.isAdmin || user.canCreate || user.isSubscriber;

        if (!allowed) throw new Error("Droit refusé.");

        // Also check if user is actually part of the chat (or is admin)
        // We open the doc to verify membership
        const doc = DocumentApp.openById(chatId);
        const meta = JSON.parse(decrypt(doc.getBody().getParagraphs()[0].getText()));
