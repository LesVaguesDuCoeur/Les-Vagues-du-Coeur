const app = {
    ...
    RECIPES_API_URL: 'https://script.google.com/macros/s/REPLACE_ME/exec', // URL du nouveau deploiement
    ...
    loadCustomRecipes: async function() {
        try {
            const response = await fetch(this.RECIPES_API_URL + '?action=loadRecipes');
            const data = await response.json();

            if (data.success && data.recipes && data.recipes.length > 0) {
                // Ajouter les recettes custom aux recettes de base
                this.state.recipes = [...this.state.recipes, ...data.recipes];
                console.log('Recettes custom chargees:', data.recipes.length);

                // Rafraichir l'affichage si on est sur la page recettes
                if (this.state.currentView === 'recipes') {
                    this.renderRecipeCatalog();
                }
            }
        } catch (error) {
            console.log('Erreur chargement recettes custom:', error);
            // Pas grave, on continue avec les recettes de base
        }
    },

    showAddRecipeModal: function() {
        const modal = document.getElementById('modal-add-recipe');
        if (!modal) {
            // Creer le modal s'il n'existe pas
            const div = document.createElement('div');
            div.id = 'modal-add-recipe';
            div.className = 'fixed inset-0 z-50 flex items-center justify-center modal-hidden';
            div.innerHTML = `
                <div class="modal-overlay absolute inset-0 bg-black/50 backdrop-blur-sm" onclick="app.closeAddRecipeModal()"></div>
                <div class="modal-content bg-white w-full max-w-lg mx-4 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto relative">
                    <div class="sticky top-0 bg-white border-b p-4 flex justify-between items-center">
                        <h3 class="text-lg font-bold">Ajouter une recette</h3>
                        <button onclick="app.closeAddRecipeModal()" class="p-2 hover:bg-gray-100 rounded-full">
                            <i data-lucide="x" class="w-5 h-5"></i>
                        </button>
                    </div>
                    <form id="add-recipe-form" class="p-4 space-y-4" onsubmit="app.submitNewRecipe(event)">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Nom de la recette *</label>
                            <input type="text" name="nom" required class="w-full p-2 border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary" placeholder="Ex: Poulet riz courgettes">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Categorie *</label>
                            <select name="categorie" required class="w-full p-2 border rounded-lg">
                                <option value="plat_complet">Plat complet (midi/soir)</option>
                                <option value="petit_dejeuner">Petit-dejeuner</option>
                                <option value="collation">Collation</option>
                                <option value="suhoor">Suhoor (Ramadan)</option>
                        ...
                        <div>
                            ...
                            <input type="text" name="tags" class="w-full p-2 border rounded-lg" placeholder="#arabe, #poulet, #rapide">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Calories (estimation)</label>
                            <input type="number" name="kcal" class="w-full p-2 border rounded-lg" placeholder="350">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Temps de cuisson</label>
                            <input type="text" name="temps_cuisson" class="w-full p-2 border rounded-lg" placeholder="15 min">
                        </div>
                        ...
                        <div>
                            ...
                            <div id="ingredients-list" class="space-y-2">
                                <div class="flex gap-2">
                                    <input type="text" name="ing_nom[]" class="flex-1 p-2 border rounded-lg" placeholder="Ingredient" required>
                                    <input type="text" name="ing_qt[]" class="w-24 p-2 border rounded-lg" placeholder="Quantite">
                                </div>
                            </div>
                            <button type="button" onclick="app.addIngredientField()" class="mt-2 text-sm text-primary hover:underline">+ Ajouter un ingredient</button>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Etapes de preparation *</label>
                            <div id="steps-list" class="space-y-2">
                                <textarea name="step[]" class="w-full p-2 border rounded-lg" rows="2" placeholder="Etape 1..." required></textarea>
                            </div>
                            <button type="button" onclick="app.addStepField()" class="mt-2 text-sm text-primary hover:underline">+ Ajouter une etape</button>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Conseils gastrite (optionnel)</label>
                            <textarea name="conseils_gastrite" class="w-full p-2 border rounded-lg" rows="2" placeholder="Conseils pour les personnes avec gastrite..."></textarea>
                        </div>

                        <button type="submit" class="w-full bg-primary text-white py-3 rounded-xl font-medium hover:bg-primary-dark">
        ...
        document.getElementById('modal-add-recipe').classList.remove('modal-hidden');
    },

    closeAddRecipeModal: function() {
        const modal = document.getElementById('modal-add-recipe');
    ...
    addIngredientField: function() {
        ...
        div.innerHTML = `
            <input type="text" name="ing_nom[]" class="flex-1 p-2 border rounded-lg" placeholder="Ingredient">
            <input type="text" name="ing_qt[]" class="w-24 p-2 border rounded-lg" placeholder="Quantite">
            <button type="button" onclick="this.parentElement.remove()" class="p-2 text-red-500 hover:bg-red-50 rounded">
    ...
    addStepField: function() {
        ...
        textarea.rows = 2;
        textarea.placeholder = `Etape ${count}...`;
        container.appendChild(textarea);
    },

    submitNewRecipe: async function(event) {
        ...
        const recipe = {
            ...
            kcal: parseInt(formData.get('kcal')) || 350,
            temps_cuisson: formData.get('temps_cuisson') || '15 min',
            ingredients: [],
            instructions: [],
            conseils_gastrite: formData.get('conseils_gastrite') || null
        };

        // Ingredients
        const ingNoms = formData.getAll('ing_nom[]');
        const ingQts = formData.getAll('ing_qt[]');
        ingNoms.forEach((nom, i) => {
            if (nom.trim()) {
                recipe.ingredients.push({ nom: nom.trim(), quantite: ingQts[i] || '' });
            }
        });
        ...
        if (recipe.ingredients.length === 0) {
            this.showToast('Ajoutez au moins un ingredient', true);
            return;
        }
        if (recipe.instructions.length === 0) {
            this.showToast('Ajoutez au moins une etape', true);
            return;
        }
        ...
        try {
            ...
            const response = await fetch(this.RECIPES_API_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'text/plain' },
                body: JSON.stringify({ action: 'addRecipe', recipe: recipe })
            });

            const result = await response.json();

            if (result.success) {
                // Ajouter la recette localement
                this.state.recipes.push(result.recipe);
                ...
                this.showToast('Erreur: ' + (result.error || 'Echec'), true);
            }
        } catch (error) {
    ...
    renderHome: function() {
        ...
        // Simple logic for next meal highlight based on time
        // This needs to adapt to Ramadan mode ideally, but simple fallback is fine
        if (!this.state.ramadanMode) {
    ...
    renderWeeklyMenu: function() {
        ...
        const theadTr = document.querySelector('#view-menu thead tr');
                ...
                if (meal && meal.skipped) {
                    ...
                    cell.innerHTML = `
                        ...
                        <button class="absolute top-1 right-1 opacity-0 group-hover:opacity-100 p-1 bg-white shadow rounded-full text-primary hover:scale-110 transition-all z-10"
                                onclick="app.showQuickMenu('${day}', '${type}', event); event.stopPropagation();" title="Modifier">
    ...
    skipMeal: function(day, mealType) {
        if (this.state.menu[day][mealType] && this.state.menu[day][mealType].skipped) {
            // Unskip (Reset to empty or default?) - Let's just randomize or clear
    ...
    addToMenuFromModal: function() {
        ...
        if (this.state.selectedDay && this.state.selectedMealType) {
            ...
            // Loose validation for Ramadan/Collation flexibility
